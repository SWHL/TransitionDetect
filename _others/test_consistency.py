...
class TestTransNetV2Consistency(unittest.TestCase):
    ...
    def test_single_model_multiple_calls(self):
        ...
        for i in range(3):
            ...
            if i == 0:
                ...
                print(
                    f"  First few scenes: {[(s['start_time'], s['end_time']) for s in first_scenes]}"
                )

        print(f"  Results: {results}")
        # All results should be identical
        self.assertEqual(
            len(set(results)), 1, f"Single model gave inconsistent results: {results}"
        )
    ...
    def test_model_state_isolation(self):
        ...
        _ = model1.get_scene_count(
            self.test_video, threshold=0.3
        )  # Different threshold
        _ = model1.get_scene_timestamps(
            self.test_video, threshold=0.7
        )  # Different threshold
    ...
    def test_wrapper_function_consistency(self):
        ...
        print(
            f"  Direct: {direct_count}, Wrapper: {wrapper_count}, Nested: {nested_count}"
        )

...
class TestAPIConsistency:
        ...
        def mock_analyze_video(video_path, threshold=0.5, quiet=False):
            ...
            for i, scene in enumerate(scenes):
                scene_data = {
                    ...
                    "probability": float(
                        np.max(test_predictions[scene[0] : scene[1] + 1])
                    ),
                }
                scene_list.append(scene_data)

            return {"scenes": scene_list, "total_scenes": len(scene_list), "fps": 25.0}
    ...
    def test_detect_scenes_matches_analyze_video(self, mock_model_setup):
        ...
        analyze_result = model.analyze_video(
            video_path, threshold=threshold, quiet=True
        )

...
class TestCLI:
    ...
    def test_cli_no_video_error(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "transnetv2_pytorch"], capture_output=True, text=True
        )
        assert result.returncode != 0
    ...
    def test_cli_auto_output_filename(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = subprocess.run(
                [sys.executable, "-m", "transnetv2_pytorch", test_video, "--quiet"],
                capture_output=True,
                text=True,
                cwd=tmpdir,
            )
            ...
            assert os.path.exists(
                expected_output
            ), "Auto-generated output file not found"
    ...
    def test_cli_nonexistent_video(self):
        ...
        assert result.returncode != 0
        assert (
            "Video file not found" in result.stdout
            or "Video file not found" in result.stderr
        )
    ...
    def test_cli_nonexistent_weights_file(self):
        ...
        assert result.returncode != 0
        assert (
            "Weights file not found" in result.stdout
            or "Weights file not found" in result.stderr
        )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = subprocess.run(
                [
                    ...
                    output_file,
                    "--format",
                    "csv",  # Explicitly specify CSV despite .json extension
                    "--quiet",
                ],
                capture_output=True,
                text=True,

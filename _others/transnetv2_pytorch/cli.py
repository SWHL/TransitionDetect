...
warnings.filterwarnings(
    "ignore", message=".*MPS backend.*will fall back to run on the CPU.*"
)
...
def process_video_to_output(
    ...
    pbar = (
        None
        if (quiet or not verbose or no_progress_bar)
        else tqdm(total=2, desc="Processing", unit="step")
    )
    ...
    try:
        ...
        results = model.analyze_video(
            video_path, threshold=threshold, quiet=(quiet or no_progress_bar)
        )
    except RuntimeError as e:
        if "MPS backend out of memory" in str(e):
            print("Error: MPS backend out of memory.")
            print(
                "The model has applied automatic memory optimizations, but your video may still exceed available memory."
            )
            print(
                "Try reducing video resolution or splitting longer videos into segments."
            )
            raise RuntimeError(
                "TransNetV2 prediction failed: MPS backend out of memory."
            ) from e
        else:
...
def get_default_weights_path():
    ...
    try:
        ...
        with resources.path(
            "transnetv2_pytorch", "transnetv2-pytorch-weights.pth"
        ) as weights_path:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process video for scene detection using TransNetV2"
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    parser.add_argument(
        "video", nargs="?", default=None, help="Path to input video file"
    )
    parser.add_argument(
        "--output",
        type=str,
    ...
    if args.video is None:
        parser.error(
            "Video file is required. Usage: python -m transnetv2_pytorch <video_file>"
        )
    ...
    if not os.path.exists(args.weights):
        ...
        if args.weights == get_default_weights_path():
            print(
                "The bundled weights file is missing. Please ensure the package is properly installed."
            )
        else:
        ...
        if args.verbose and not args.quiet:
            ...
            if len(data) > 0:
                ...
                for i, item in enumerate(data[:5]):
                    print(
                        f"  Shot {item['shot_id']}: frames {item['start_frame']}-{item['end_frame']} "

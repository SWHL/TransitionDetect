...
class TransNetV2(nn.Module):
    ...
    def predict_frames(self, frames, quiet=False):
        ...
        def input_iterator():
            ...
            no_padded_frames_end = (
                ...
                + step_size
                - (
                    len(frames) % step_size
                    if len(frames) % step_size != 0
                    else step_size
                )
            )
        ...
        pbar = (
            None
            if quiet
            else tqdm(total=len(frames), desc="Processing frames", unit="frame")
        )
    ...
    def predictions_to_scenes_with_data(
        ...
        for i, scene in enumerate(scenes):
            ...
            max_probability = (
                float(np.max(scene_probs)) if len(scene_probs) > 0 else 0.0
            )
    ...
    def analyze_video(
        self, video_path: str, threshold: float = 0.5, quiet: bool = False
    ) -> Dict[str, Any]:
        """
    ...
    def get_scene_timestamps(
        ...
        scenes = self.detect_scenes(video_path, threshold)
        return [
            (float(scene["start_time"]), float(scene["end_time"])) for scene in scenes
        ]
